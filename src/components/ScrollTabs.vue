...
const props = defineProps<
  TabsRootProps & { class?: HTMLAttributes["class"] } & { items: TabItem[] }
>();
...
watch(
  ...
  async () => {
    await nextTick();
    const activeTab = scrollRef.value?.querySelector('[data-state="active"]') as HTMLElement;
      ...
      <div class="relative flex min-w-0 flex-1 overflow-hidden">
        ...
        <div class="no-scrollbar flex-1 overflow-x-auto" ref="scrollRef">
          <TabsList class="relative flex w-max gap-0 px-2">
            ...
            <TabsTrigger
              v-for="{ value, label, disabled, badge } in tabItems"
              :key="value"
              :value
              ...
              :disabled
            >
              {{ label }}<Badge v-if="badge" class="ml-1 px-1 py-0 text-xs">{{ badge }}</Badge>
            </TabsTrigger>
